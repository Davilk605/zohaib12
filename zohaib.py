...
def helpnote():
	...
	print("%s [*] JOIN MY FB GROUP  To KNOW ABOUT UPDATES  :)"%(G))

	subprocess.check_output(["am", "start", "https://facebook.com/groups/690029042050227/"])

	exit(" [*] Whatsapp :  https://wa.me/+923134445926")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923134445926")

	subprocess.check_output(["am", "start", "https://wa.me/+923134445926"])
...
class Main:
	...
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://github.com/Davilk605/zohaib12/main/paid.txt').text
...
.##.....##.####.########...#######.....##...
   ┌───────────────────────────────────────┐
    ...
    │ [✓] WHATSAPP :  +923134445926     │

    │ [✓] POWER BY : \x1b[1;32mZOHAIB ULLAH\x1b[1;97m             │

    └───────────────────────────────────────┘  """)

		print("%s [%s®%s] %sTOOL NAME : %sOLD CLONE"%(G,R,G,Y,G))

		print("%s [%s®%s] %sVERSION   : %s1.1 "%(G,R,G,Y,G))
		...
		print("\033[92;1m         FERDOUS ZOHAIB      ")

		print("\033[97;1m   ───────────────────────────────────────")

		print("\n%s [%s1%s]%s CRACK RANDOM FB ID 2008-11 %s(FREE)"%(G,R,G,Y,W))

		print("%s [%s2%s]%s CRACK FB ID 2015-22 %s(PAID 50 TAKA) V1"%(G,R,G,Y,G))

		print("%s [%s3%s]%s CRACK FROM NUMBER  %s(PAID 100 TAKA) VIP"%(G,R,G,Y,G))

		print("%s [%s4%s]%s CRACK FB ID 2004-5%s(150 TAKA) VIP-1"%(G,R,G,Y,G))

		print("%s [%s5%s]%s CRACK FROM EMAILS %s(150 TAKA)"%(G,R,G,Y,G))

		print("%s [%s6%s]%s CRACK FB ID Custom %s(200 TAKA) V1"%(G,R,G,Y,G))
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [!] USE %s, %s(COMMA)%s FOR PRONOY "%(Y,G,B,Y))
	...
	def old4_6(self):
		...
		limit = int(input("\033[092m [+] ENTER LIMIT \033[0;91m(10000 MAX): \033[0;92m"))
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\n%s [!] USE %s, %s(COMMA)%s FOR FERDOUS "%(Y,G,B,Y))
	...
	def email(self):
		...
		nam = input("%s [?] TYPE A NAME %s(EX:Promoy ", " Shahin): "%(Y,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			...
			"\r\r %s\033[0;93m[FERDOUS] : \033[0;97m %s/%s -> \033[0;92m [OK:%s ]- \033[0;93m[CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
